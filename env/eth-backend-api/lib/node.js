...
console.success('   ', 'v' + packJson.version);
console.info('   ');
console.info('   ');
...
Node.prototype.validateLatestBlock = function (error, result, timeString) {
	...
	if(this.stats.block.number - this._lastBlock > 1) {
		const range = _.range(Math.max(this.stats.block.number - MAX_BLOCKS_HISTORY, this._lastBlock + 1), Math.max(this.stats.block.number, 0), 1);
...
Node.prototype.setFilters = function() {
	...
	try {
		...
		{
			const now = _.now();
			const time = now - self._lastChainLog;
